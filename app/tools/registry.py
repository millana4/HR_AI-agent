...
_SEARCH_ATS_MAVIS_SPEC = ToolSpec(
    ...
    description=(
        "Найти ТЕЛЕФОН (внутренний/добавочный номер) отдела компании Мавис. "
        "Только когда пользователь явно просит позвонить или узнать номер отдела. "
        "НЕ использовать для вопросов 'как заказать', 'как сделать', 'где найти', "
        "'куда обратиться' — для них есть search_internal."
    ),
    parameters={
        "type": "object",
        "properties": {
...
_SEARCH_ATS_VOTONIA_SPEC = ToolSpec(
    ...
    description=(
        "Найти ТЕЛЕФОН (внутренний/добавочный номер) отдела компании Вотоня. "
        "Только когда пользователь явно просит позвонить или узнать номер отдела. "
        "НЕ использовать для вопросов 'как заказать', 'как сделать', 'где найти', "
        "'куда обратиться' — для них есть search_internal."
    ),
